...
def recompute_holders(template) -> RecomputeResult:
    ...
    # A template with no rules is managed manually (e.g. a manually-assigned badge). There is no
    # rule-based notion of who should hold it, so recompute must leave its holders untouched —
    # otherwise it would treat every manual grant as "no longer qualifies" and revoke it.
    if not rules:
        logger.info('Skipping recompute for rule-less template %r (manual assignment only).', template)
        return result
