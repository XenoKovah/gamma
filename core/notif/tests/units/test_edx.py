...
def test_send_notif(push_factory, notif_data, notif_cfg, user, mocker):
    """
    Check sending.
    """
    ...
    edx_provider = push_factory.get(Provider.EDX, **notif_cfg)
...
@pytest.mark.unittests
def test_disabled_send_notif(push_factory, notif_data, notif_cfg, user, mocker):
    """
...
@pytest.mark.unittests
def test__create_base_notif(push_factory, user, notif_data, notif_cfg):
    edx_provider = push_factory.get(Provider.EDX, **notif_cfg)
    ...
    assert notif.message == json.dumps({
        "title": notif_data['head'],
        "description": notif_data['body'],
    })
...
def test__create_base_notif__kinds(push_factory, user, notif_data, notif_cfg, kind):
...
@pytest.mark.unittests
def test__default_format_message(push_factory, notif_data, notif_cfg):
    notif_cfg.pop(Config.EDX_NOTIF_FORMAT_FUNC)
    edx_provider = push_factory.get(Provider.EDX, **notif_cfg)
...
@pytest.mark.unittests
def test__json_format_message(push_factory, notif_data, notif_cfg):
    edx_provider = push_factory.get(Provider.EDX, **notif_cfg)
    text = edx_provider._format_message(notif_data)

    assert text == json.dumps({
        "title": notif_data.get("head"),
        "description": notif_data.get("body"),
    })


@pytest.mark.unittests
def test__default_format_message_broken_head(push_factory, notif_data, notif_cfg):
    notif_cfg.pop(Config.EDX_NOTIF_FORMAT_FUNC)
    edx_provider = push_factory.get(Provider.EDX, **notif_cfg)
...
@pytest.mark.unittests
def test__json_format_message_broken_head(push_factory, notif_data, notif_cfg):
    edx_provider = push_factory.get(Provider.EDX, **notif_cfg)
...
@pytest.mark.unittests
def test__default_format_message_broken_body(push_factory, notif_data, notif_cfg):
    notif_cfg.pop(Config.EDX_NOTIF_FORMAT_FUNC)
    edx_provider = push_factory.get(Provider.EDX, **notif_cfg)
...
@pytest.mark.unittests
def test__json_format_message_broken_body(push_factory, notif_data, notif_cfg):
    edx_provider = push_factory.get(Provider.EDX, **notif_cfg)
