...
class BadgeCSVImportService:
    ...
    def __init__(self, dry_run: bool = False, delete: bool = False):
    ...
    def _process_row(self, idx: int, row: Dict[str, str]) -> None:
        ...
        if self.delete and slug:
            logger.info("Processing badge deletion slug=%s", slug)
            if self.dry_run:
                logger.info("DRY-RUN: would delete badge %s", slug)
                return

            deleted, _ = Badge.objects.filter(slug=slug).delete()
            if deleted:
                logger.info("Deleted badge %s", slug)
            else:
                logger.warning("Badge %s not found for deletion", slug)
            return

        if not slug or not title or not image_path:
            logger.warning("Skipping line %s: missing required data", idx)
            return

        if not image_path.exists():
            logger.warning("Skipping line %s: badge image does not exist (%s)", idx, image_path)
            return
