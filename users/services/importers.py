...
class UserCSVImportService:
    ...
    TODO: Create interface for import services to standardize usage.
    """

    def __init__(self, *, dry_run: bool = False, delete: bool = False) -> None:
    ...
    def run(self, csv_path: Path) -> None:
        ...
        for idx, row in self._iterate_rows(csv_path):
    ...
    def _iterate_rows(self, csv_path: Path) -> Iterator[Tuple[int, Dict[str, str]]]:
        with csv_path.open(mode="r", encoding="utf-8", newline="") as csv_file:
            ...
            for idx, row in enumerate(self._reader, start=2):  # 1=header
                yield idx, row

    def _process_row(self, idx: int, row: Dict[str, str]) -> None:
        """
        ...
        if not user_uid:
            logger.warning("Skipping line %s: missing user_uid", idx)
            return
        
        if self.delete:
            if self.dry_run:
                logger.info("[DRY-RUN] would delete user_uid=%s", user_uid)
                return

            deleted, _ = GammaUser.objects.filter(user_uid=user_uid).delete()

            if deleted:
                logger.info("Deleted user_uid=%s", user_uid)
            else:
                logger.warning("User to delete not found user_uid=%s", user_uid)
        ...
        try:
            ...
            logger.warning("Skipping line %s: invalid points value %r", idx, row.get("points"))
            return
        ...
        with transaction.atomic():
            try:
                ...
                logger.warning(
                    "Skipping line %s: could not create/update user_uid=%s.",
                    idx,
                    user_uid,
                )
                return
